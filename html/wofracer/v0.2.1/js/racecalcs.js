function calculateRaceTime(raceName, raceTerrain, raceWeather, vehicleTokenId, vehicleName, cargoWeight, raceDistance, vehicleStats, vehicleAdjustableKO)
{
    ...
    const terrainFactor = ((raceDistance >= vehicleAdjustableTerrain.timeFactor.rangeLimitLower) && (raceDistance <= vehicleAdjustableTerrain.timeFactor.rangeLimitUpper)) ? vehicleAdjustableTerrain.timeFactor.factor : 1;
    const terrainFactorDisqualifier = vehicleAdjustableTerrain.timeFactor.disqualifyOutsideRange;
    const weatherFactor = ((raceDistance >= vehicleAdjustableWeather.timeFactor.rangeLimitLower) && (raceDistance <= vehicleAdjustableWeather.timeFactor.rangeLimitUpper)) ? vehicleAdjustableWeather.timeFactor.factor : 1;
    const weatherFactorDisqualifier = vehicleAdjustableWeather.timeFactor.disqualifyOutsideRange;

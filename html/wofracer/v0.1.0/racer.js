...
function joinRace()
{
    ...
    while ((!joined) && (i < unjoinedRacesVM.unjoinedRaceList().length))
    {
        ...
        i++;
        if ((participantCount >= participationThreshold) && (unjoinedRace.selectedVehicle.vehicleTokenId()))
        {
            joined = true;
    ...
    $.each(unjoinedRaces, function(index)
        {
            ...
            unjoinedRaces[index].raceDistance = Math.round(unjoinedRaces[index].distance);
        }
    );
...
ROOT_GRAPH_URL = 'https://graph.worldoffreight.xyz/v1';

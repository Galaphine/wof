...
{
    const offset = (seconds < 3600) ? 3 : 0;
    if (seconds == Infinity) return seconds;

    dayPortion = '00';
    if (seconds > 3600*24) dayPortion = String(parseInt(seconds/(3600*24), 10)).padStart(2, '0');
    const dateString = new Date(seconds * 1000).toISOString();
    return dayPortion + ':' + ((offset == 3) ? '00:' : '') + dateString.substring(11 + offset, 20 + offset);
}
